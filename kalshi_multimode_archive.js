/**
 * kalshi_multimode_archive.js — ARCHIVE / NOT RUN IN PRODUCTION
 *
 * Full multi-mode research bot (Modes 1, 2, 3, 4, 4a, 4b). Kept for
 * ongoing analysis and as a reference for the paper strategies and the
 * contract-capture system. The LIVE bot (kalshi_btc_paper_bot.js) runs
 * ONLY Mode 4a. This file is intentionally NOT referenced by any
 * workflow. Do not wire it into CI without giving it a separate state
 * file — it shares dashboard/btc_paper_state.json and would clobber the
 * live bot's state.
 *
 * ── Original modes ──
 ...
 *
 * MODE 4 / 4a / 4b — 7-TF confluence (4a is now the live strategy)
 *
 * CONTRACT CAPTURE SYSTEM
 *   Logs real Kalshi YES/NO prices, BTC price, distance, RSI at open,
 *   entry, and resolution phases for later analysis.
 */
...
// ── Mode 4a: Value Entries (works on both 15-min and hourly) ──────
// 15-min contracts (KXBTC15M): strike = BTC at window open, so
//   prices start ~50¢ and only gradually diverge — ideal for value.
// Hourly contracts (KXBTCD): at 10-30 min left, closest strike
//   may have moderate odds.
// Window: 1-30 min left — the mid-price filter (50-85¢) is the real
//   gate, not time remaining.  A 79¢ contract at 4 min left is just
//   as good as one at 12 min left.
// Recovery: after a loss, skip 2 signals, then bet 3x
const MODE4A_BET        = 5.00;
...
const MODE4A_CONFLUENCE = 2;

// ── Mode 4b: Late-Window High-Probability Entries ─────────────────
// Entry window : 9–14.5 min into contract (0.5–6 min remaining)
// Mid filter   : 86–96¢ — high probability, small payouts
// Confluence   : 3+ of 7 TFs agree
// Recovery     : after a loss, skip 2 signals, then bet 3x
const MODE4B_BET        = 5.00;
const MODE4B_MID_LO     = 0.86;
const MODE4B_MID_HI     = 0.96;
const MODE4B_MIN_IN     = 9;
const MODE4B_MAX_IN     = 14.5;
const MODE4B_MIN_LEFT   = 0.5;
const MODE4B_MAX_LEFT   = 6;
const MODE4B_CONFLUENCE = 3;

// ── Recovery: skip 2 signals after loss, 3x on comeback ──────────
const RECOVERY_SKIP     = 2;
const RECOVERY_MULTI    = 3;
...
// ── After loss/win — update streak and pause/recovery logic ───────
function updateStreak(modeState, won, modeKey) {
  const is4ab = modeKey === 'mode4a' || modeKey === 'mode4b';

  if (won) {
    modeState.lossStreak = 0;
    if (is4ab) {
      modeState.recoveryMulti = 1;
    } else if (modeState.useReducedBet) {
      ...
      if (modeState.reducedWins >= WINS_TO_RESTORE) {
        ...
        console.log(`  [${modeKey.toUpperCase()}] 2 wins in a row — restored to full bet`);
      }
    }
  } else {
    modeState.lossStreak = (modeState.lossStreak || 0) + 1;
    if (is4ab) {
      modeState.skipSignals = RECOVERY_SKIP;
      modeState.recoveryMulti = RECOVERY_MULTI;
      console.log(`  [${modeKey.toUpperCase()}] Loss — skipping next ${RECOVERY_SKIP} signals, then ${RECOVERY_MULTI}x bet`);
    } else {
      modeState.reducedWins = 0;
...
// ── MODE 4: 7-TF Confluence Evaluator (parameterized) ─────────────
// cfg: { midLo, midHi, minIn, maxIn, minLeft, maxLeft, confluence }
function evalMode4(mkt, candlesByTf, minutesIn, minutesLeft, cfg) {
  if (minutesIn < cfg.minIn || minutesIn > cfg.maxIn ||
      minutesLeft < cfg.minLeft || minutesLeft > cfg.maxLeft) {
  ...
  if (!yesOk && !noOk) {
    ...
    return { enter: false, reason: `Mid out of range (best ${best}¢ — need ${cfg.midLo*100}-${cfg.midHi*100}¢)` };
  }

  const { dir, score, breakdown } = calcConfluence(candlesByTf, cfg.confluence);
  if (!dir) {
    return { enter: false, reason: `Weak confluence (${score}/7 TFs — need ${cfg.confluence}+)` };
  }
...
function ensureStateShape(state) {
  ...
  state.mode4a = { ...defaultModeState(70), skipSignals: 0, recoveryMulti: 1, ...(state.mode4a || {}) };
  state.mode4b = { ...defaultModeState(PAPER_START), skipSignals: 0, recoveryMulti: 1, ...(state.mode4b || {}) };
  // Guarantee arrays exist even if loaded JSON had them missing or null
  for (const mk of ['mode1', 'mode2', 'mode3', 'mode4', 'mode4a', 'mode4b']) {
...
async function enterPosition(modeState, modeKey, entry) {
  ...
  // Only Mode 4a/4b trade live — all other modes are paper-only
  const isLive = LIVE_MODE && modeKey === 'mode4a';
...
async function scan() {
  ...
  // KXBTCD   = hourly/daily range contracts (above/below strike)
  // KXBTC15M = 15-min up/down contracts (floor_strike = BTC at open)
  const BTC_SERIES = ['KXBTCD', 'KXBTC15M'];
  let allMkts = [];
  try {
    const seriesResults = await Promise.all(
      BTC_SERIES.map(s => kalshiGet(`/events?series_ticker=${s}&status=open&limit=10`).catch(() => ({})))
    );
    const events = [];
    for (let i = 0; i < BTC_SERIES.length; i++) {
      const evts = seriesResults[i].events || [];
      if (evts.length > 0) {
        console.log(`  📡 ${BTC_SERIES[i]}: ${evts.length} event(s)`);
        events.push(...evts);
      }
    }
    if (events.length === 0) console.log('  !! No BTC events found across any series');
    ...
    // Log contract duration breakdown so we can see what's available
    const durations = {};
    for (const m of allMkts) {
      if (!m.close_time || !m.open_time) continue;
      const dur = Math.round((new Date(m.close_time) - new Date(m.open_time)) / 60000);
      durations[dur] = (durations[dur] || 0) + 1;
    }
    if (Object.keys(durations).length > 0) {
      const durStr = Object.entries(durations).sort((a,b) => a[0]-b[0])
        .map(([d,n]) => `${d}min:${n}`).join('  ');
      console.log(`  📊 Contract durations: ${durStr}`);
    }
  } catch (e) {
  ...
  await Promise.all([
    ...
    resolveExpired(state.mode4a, 'mode4a', allMkts),
    resolveExpired(state.mode4b, 'mode4b', allMkts),
  ]);
  ...
  // -- MODE 4a + 4b: Dual-window confluence strategy ----------------
  // 4a: mid-window (10-30min left) at 50-85¢ — value entries
  // 4b: late window (0.5-6min left) at 86-96¢ — high probability
  // Both: skip 2 signals after loss, then 3x recovery bet
  {
    const subModes = [
      { key: 'mode4a', label: 'M4a', baseBet: MODE4A_BET, cfg: {
        midLo: MODE4A_MID_LO, midHi: MODE4A_MID_HI,
        minIn: MODE4A_MIN_IN, maxIn: MODE4A_MAX_IN,
        minLeft: MODE4A_MIN_LEFT, maxLeft: MODE4A_MAX_LEFT,
        confluence: MODE4A_CONFLUENCE,
      }},
      { key: 'mode4b', label: 'M4b', baseBet: MODE4B_BET, cfg: {
        midLo: MODE4B_MID_LO, midHi: MODE4B_MID_HI,
        minIn: MODE4B_MIN_IN, maxIn: MODE4B_MAX_IN,
        minLeft: MODE4B_MIN_LEFT, maxLeft: MODE4B_MAX_LEFT,
        confluence: MODE4B_CONFLUENCE,
      }},
    ];

    for (const { key, label, baseBet, cfg } of subModes) {
      const ms = state[key];
      const paused = ms.pauseUntil && Date.now() < ms.pauseUntil;
      if (paused) {
        const minsLeft = ((ms.pauseUntil - Date.now()) / 60000).toFixed(0);
        console.log(`\n  [${label}] PAUSED -- ${minsLeft}min remaining`);
        continue;
      }
      ...
      const candidates = allMkts
        ...
        .filter(({ minutesIn, minutesLeft }) =>
          minutesIn >= cfg.minIn && minutesIn <= cfg.maxIn &&
          minutesLeft >= cfg.minLeft && minutesLeft <= cfg.maxLeft
        );

      if (candidates.length === 0) {
        console.log(`\n  [${label}] No markets in window this scan`);
      }

      for (const { m, minutesIn, minutesLeft } of candidates) {
        if (ms.open.some(p => p.ticker === m.ticker)) continue;

        const sig = evalMode4(m, candlesByTf, minutesIn, minutesLeft, cfg);
        if (!sig.enter) {
          console.log(`  [${label}] SKIP ${m.ticker.slice(-20)} -- ${sig.reason}`);
          continue;
        }

        // Recovery logic: skip signals after a loss, then bet 3x
        if (ms.skipSignals > 0) {
          ms.skipSignals--;
          console.log(`  [${label}] SKIP (recovery cooldown, ${ms.skipSignals} left) -- ${sig.reason}`);
          continue;
        }

        const multi = ms.recoveryMulti || 1;
        const cost  = baseBet * multi;
        if (ms.bankroll < cost) {
          console.log(`  [${label}] SKIP -- bankroll $${ms.bankroll.toFixed(2)} < bet $${cost.toFixed(2)}`);
          continue;
        }

        const profit = cost * (1 / sig.contractPx - 1);
        const multiTag = multi > 1 ? ` [${multi}x RECOVERY]` : '';
        console.log(`\n  [${label}] ENTER -- ${sig.reason}  win: +$${profit.toFixed(2)}${multiTag}`);

        const entered = await enterPosition(ms, key, {
          ticker: m.ticker, side: sig.side, cost, contractPx: sig.contractPx,
          payout: cost / sig.contractPx,
          mode: label, confluenceScore: sig.confluenceScore, confluenceDir: sig.confluenceDir,
          btcPrice, strike: m.floor_strike, distance: btcPrice - m.floor_strike,
          rsi1m, close_time: m.close_time, recoveryMulti: multi,
        });
        if (entered) {
          const tag = LIVE_MODE ? '[LIVE]' : '[PAPER]';
          console.log(`  [${label}] >> ${tag} ${sig.side} @ ${(sig.contractPx*100).toFixed(0)}c  bet $${cost.toFixed(2)}  payout $${(cost/sig.contractPx).toFixed(2)}`);
        }
      }
    }
  }

  // -- MODE 4 (legacy): still resolves old positions, no new entries --
  // Kept so existing open positions resolve cleanly.
  ...
  for (const [mk, label] of [['mode1','Mode 1'],['mode2','Mode 2'],['mode3','Mode 3']]) {
    ...
    const betStr   = mk !== 'mode3' ? `  bet $${curBet.toFixed(2)} [paper]` : '';
    console.log(`  ${label}: ${s.wins}/${s.tot} (${s.wr.toFixed(0)}% WR)  P&L ${fmt$(s.pnl)}  streak: ${ms.lossStreak}L${bankStr}${betStr}${pauseStr}`);
  }

  // Mode 4a + 4b stats
  for (const [mk, label, base] of [['mode4a', 'Mode 4a', MODE4A_BET], ['mode4b', 'Mode 4b', MODE4B_BET]]) {
    const s  = modeStats(state[mk]);
    const ms = state[mk];
    const multi = ms.recoveryMulti || 1;
    const curBet = base * multi;
    const skipStr = ms.skipSignals > 0 ? `  SKIP ${ms.skipSignals}` : '';
    const multiStr = multi > 1 ? ` [${multi}x]` : '';
    const liveTag = LIVE_MODE ? ' LIVE' : '';
    console.log(`  ${label}: ${s.wins}/${s.tot} (${s.wr.toFixed(0)}% WR)  P&L ${fmt$(s.pnl)}  streak: ${ms.lossStreak}L  bank $${ms.bankroll.toFixed(2)}  bet $${curBet.toFixed(2)}${multiStr}${liveTag}${skipStr}`);
  }

  // Legacy Mode 4 — show if it has history
  {
    const s4 = modeStats(state.mode4);
    if (s4.tot > 0) {
      console.log(`  Mode 4 (legacy): ${s4.wins}/${s4.tot} (${s4.wr.toFixed(0)}% WR)  P&L ${fmt$(s4.pnl)}  [no new entries]`);
    }
  }
...
console.log(`  Bet: $${MODE4A_BET} (4a)  $${MODE4B_BET} (4b)  |  Recovery: skip ${RECOVERY_SKIP} then ${RECOVERY_MULTI}x`);
